...
input=/user/azkaban/camus/userclick/hourly/$day/$hour/*
...
java -classpath $bin:$CLASSPATH $JAVA_OPTS yidian.data.neo.mr.CommitHadoopJob -Dhdfs.input=$input -Dversion=$version -Dmatrix.version.base=$baseVersion -Dinput.type=hdfs -Dmapreduce.job.reduces=17 -Dmerge.type=$merge_type -Dmapreduce.reduce.memory.mb=1200 -Dmapred.reduce.child.java.opts=-Xmx1100m -Dscheduler.matrix.list=UidDoc,DocChannel,UserGroupProfile -Dmapreduce.map.speculative=false
